(function () {
  ...
  function initHero() {
    ...
    window.addEventListener('scroll', () => {
      ...
      rafPending = true;
      requestAnimationFrame(() => { animateHero(); rafPending = false; });
    }, { passive: true });
  ...
  function initCarousel() {
    ...
    const track = document.getElementById('cap-track-001');
    if (!track) return;

    [...capabilities, ...capabilities].forEach(item => track.appendChild(buildCard(item)));
  }
  ...
  function initFAQ() {
    document.querySelectorAll('.faq-question').forEach(question => {
      question.addEventListener('click', () => {
        ...
        if (!isOpen) clickedItem.classList.add('is-open');
      });
    });
  }
